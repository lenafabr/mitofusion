MODULE MITOUTIL
  ...
  TYPE CLUSTERSET
     ...
     INTEGER :: MAXCLUSTID ! maximum used position of a live cluster (NOT the id tag)
     ...
     DOUBLE PRECISION :: PS,KW, KPROD, UNITPROT, &
          & VEL, KDECAY, L, SINKWIDTH
...
CONTAINS
  ...
  SUBROUTINE MAKENEWCLUSTER(CLUSTP, CNEW)
    ...
    ! find the new first empty position in thelist
    FIRSTSET = .FALSE.
  ...
  SUBROUTINE INITIALIZECLUSTERSET(CLUSTP)
    USE KEYS, ONLY : PS, KW, KPROD, UNITPROT, &
  ...
  SUBROUTINE CLEANUPCLUSTERSET(CLUSTP)
    ...
    DEALLOCATE(CLUSTP%POS, CLUSTP%SIZE, CLUSTP%STATE, CLUSTP%PROT, &
         & CLUSTP%EXISTS, CLUSTP%DEAD, CLUSTP%IDTAG)
