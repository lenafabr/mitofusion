...
CONTAINS
  SUBROUTINE RUNDYNAMICS(CLUSTERLIST, NSTEP, NITER, NREGIONS, &
    ...
    DOUBLE PRECISION :: U, MAKEPROB, DECAYMULT, CURTIME, &
         & FISSPROB, FUSPROBMOV, FUSPROBSTAT, PAUSEPROB,&
    ...
    !FISSPROB = 1D0-EXP(-CLUSTP%KFISSION*DELT)
    !FUSPROB = 1D0-EXP(-CLUSTP%KFUSION*DELT)
    ! fussion probability should have nothing to do with delt if the mitochondria are point particles
    FUSPROBMOV = CLUSTP%KFMOV
    FUSPROBSTAT = CLUSTP%KFSTAT

    ! PAUSEPROB = 1D0-EXP(-CLUSTP%PAUSERATE*DELT)
    !    WALKPROB = 1D0-EXP(-CLUSTP%WALKRATE*DELT)
...
!          CLUSTP%PROT(CNEW) = GRND()*(CLUSTP%UNITPROT-DEATHPROTLEVEL) + DEATHPROTLEVEL
    ...
    WRITE(88,*) NSINKS, CLUSTP%KPROD,CLUSTP%KFMOV,CLUSTP%KFSTAT,&
         & CLUSTP%VEL, NREGIONS, MODELFLAG, CLUSTP%KDECAY
    STEP = 0
    ...
    DO STEP = 1,NSTEP       
       ...
       DO ITER = 1,NITER
         ...
         !      & SQRT(SUM(CLUSTP%PROT(1:10)**2)/10 - (SUM(CLUSTP%PROT(1:10))/10)**2)
          ...
          ! --------------
          ! New code for fusion, resolving spatial order of mitochondria
          ! --------------
          DO RC = 1,NREGIONS
             NCROSS = 0
             NINREG = 0
